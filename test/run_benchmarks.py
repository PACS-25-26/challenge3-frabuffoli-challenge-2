...
def collect_hw():
    ...
    for label, cmd in [("uname", ["uname", "-a"]),
        ...
        out.append("")
    (TEST_DIR / "hw.info").write_text("\n".join(out))


# ── Main ──────────────────────────────────────────────────────────────────────

BENCHMARKS = {
    1: ("1_strong_scaling", bench_strong, "strong"),
    2: ("2_weak_scaling", bench_weak, "weak"),
    3: ("3_hybrid", bench_hybrid, "hybrid"),
    4: ("4_jacobi_vs_schwarz", bench_jvs, "jvs"),
    5: ("5_accuracy_L2", bench_accuracy, "accuracy"),
}
